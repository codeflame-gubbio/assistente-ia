...
class Assistente_IA_Admin_RAG {
    ...
    public static function render(){
        ...
        if ( isset($_POST['assia_svuota_embeddings']) && current_user_can('manage_options') ) {
            ...
            // Reset anche i flag di indicizzazione
            delete_option('assia_woo_products_indexed');
            delete_option('assia_content_snapshot');
            delete_option('assia_snapshot_initialized');
        ...
        if ( $stats['totale'] > 0 ) {
            ...
            if ( $stats['prodotti_count'] > 0 ) {
                echo '<div class="assia-stat-card">';
                echo '<h3>Prodotti WooCommerce</h3>';
                echo '<div class="assia-stat-value">' . $stats['prodotti_count'] . '</div>';
                echo '<div class="assia-stat-label">Prodotti indicizzati</div>';
                echo '</div>';
            }
            ...
            // Verifica se ci sono prodotti WooCommerce
            if ( class_exists('WooCommerce') ) {
                $product_count = wp_count_posts('product');
                if ( $product_count->publish > 0 ) {
                    echo '<br><br><strong>📦 Rilevati ' . $product_count->publish . ' prodotti WooCommerce</strong> che verranno indicizzati automaticamente.';
                }
            }
        ...
        $woo_indexed = get_option('assia_woo_products_indexed', false);
        $snapshot_init = get_option('assia_snapshot_initialized', false);
        
        echo '<div style="background:#f0f0f0;padding:10px;border-radius:6px;margin:10px 0;font-size:12px">';
        echo '<strong>🔍 Stato Sistema:</strong><br>';
        echo '• Post selezionati: ' . count($selected_posts) . '<br>';
        echo '• Pagine selezionate: ' . count($selected_pages) . '<br>';
        
        if ( class_exists('WooCommerce') ) {
            $product_count = wp_count_posts('product');
            echo '• Prodotti WooCommerce: ' . $product_count->publish;
            echo $woo_indexed ? ' (già indicizzati almeno una volta)' : ' (mai indicizzati)';
            echo '<br>';
        } else {
            echo '• WooCommerce: Non attivo<br>';
        }
        
        echo '• Snapshot inizializzato: ' . ($snapshot_init ? 'Sì' : 'No') . '<br>';
        echo '• Auto-regenerate: ' . (get_option('assia_auto_regenerate_hash', 'si') === 'si' ? 'Attivo' : 'Disattivo');
        echo '</div>';
        ...
        // Lista dei contenuti indicizzati
        if ( $stats['totale'] > 0 ) {
            $contents = self::get_indexed_contents();
            
            echo '<details style="margin-top:20px">';
            echo '<summary style="cursor:pointer;padding:10px;background:#f3f4f6;border-radius:6px;font-weight:600">📋 Mostra contenuti indicizzati (' . (count($contents['posts']) + count($contents['prodotti'])) . ')</summary>';
            echo '<div style="margin-top:15px;max-height:400px;overflow:auto;border:1px solid #e5e7eb;border-radius:6px;padding:15px">';
            
            if ( ! empty($contents['posts']) ) {
                echo '<h4 style="margin:0 0 10px 0">📄 Pagine e Articoli (' . count($contents['posts']) . ')</h4>';
                echo '<table style="width:100%;border-collapse:collapse">';
                echo '<thead><tr>';
                echo '<th style="text-align:left;padding:5px;border-bottom:2px solid #e5e7eb">ID</th>';
                echo '<th style="text-align:left;padding:5px;border-bottom:2px solid #e5e7eb">Tipo</th>';
                echo '<th style="text-align:left;padding:5px;border-bottom:2px solid #e5e7eb">Titolo</th>';
                echo '<th style="text-align:center;padding:5px;border-bottom:2px solid #e5e7eb">Chunks</th>';
                echo '<th style="text-align:center;padding:5px;border-bottom:2px solid #e5e7eb">Link</th>';
                echo '</tr></thead>';
                echo '<tbody>';
                foreach ( $contents['posts'] as $item ) {
                    echo '<tr>';
                    echo '<td style="padding:5px;border-bottom:1px solid #f3f4f6">' . $item['id'] . '</td>';
                    echo '<td style="padding:5px;border-bottom:1px solid #f3f4f6">' . $item['type'] . '</td>';
                    echo '<td style="padding:5px;border-bottom:1px solid #f3f4f6">' . esc_html($item['title']) . '</td>';
                    echo '<td style="text-align:center;padding:5px;border-bottom:1px solid #f3f4f6">' . $item['chunks'] . '</td>';
                    echo '<td style="text-align:center;padding:5px;border-bottom:1px solid #f3f4f6"><a href="' . esc_url($item['link']) . '" target="_blank">🔗</a></td>';
                    echo '</tr>';
                }
                echo '</tbody></table>';
            }
            
            if ( ! empty($contents['prodotti']) ) {
                echo '<h4 style="margin:20px 0 10px 0">🛍️ Prodotti WooCommerce (' . count($contents['prodotti']) . ')</h4>';
                echo '<table style="width:100%;border-collapse:collapse">';
                echo '<thead><tr>';
                echo '<th style="text-align:left;padding:5px;border-bottom:2px solid #e5e7eb">ID</th>';
                echo '<th style="text-align:left;padding:5px;border-bottom:2px solid #e5e7eb">Nome Prodotto</th>';
                echo '<th style="text-align:center;padding:5px;border-bottom:2px solid #e5e7eb">Chunks</th>';
                echo '<th style="text-align:center;padding:5px;border-bottom:2px solid #e5e7eb">Link</th>';
                echo '</tr></thead>';
                echo '<tbody>';
                foreach ( $contents['prodotti'] as $item ) {
                    echo '<tr>';
                    echo '<td style="padding:5px;border-bottom:1px solid #f3f4f6">' . $item['id'] . '</td>';
                    echo '<td style="padding:5px;border-bottom:1px solid #f3f4f6">' . esc_html($item['title']) . '</td>';
                    echo '<td style="text-align:center;padding:5px;border-bottom:1px solid #f3f4f6">' . $item['chunks'] . '</td>';
    ...
    /**
     * Recupera la lista dei contenuti indicizzati con dettagli
     */
    protected static function get_indexed_contents(): array {
        global $wpdb;
        $pref = $wpdb->prefix;
        $modello = get_option('assia_modello_embedding', 'text-embedding-005');
        
        $results = $wpdb->get_results( $wpdb->prepare(
            "SELECT DISTINCT fonte, id_riferimento, COUNT(*) as chunks 
             FROM {$pref}assistente_ia_embeddings 
             WHERE modello=%s 
             GROUP BY fonte, id_riferimento 
             ORDER BY fonte, id_riferimento",
            $modello
        ), ARRAY_A );
        ...
        foreach ( $results as $row ) {
            ...
            if ( $row['fonte'] === 'prodotto' && function_exists('wc_get_product') ) {
                $product = wc_get_product( $row['id_riferimento'] );
                if ( $product ) {
                    $item['title'] = $product->get_name();
                    $item['link'] = get_permalink( $row['id_riferimento'] );
                    $item['type'] = 'Prodotto';
                    $contents['prodotti'][] = $item;
                }
            } else {
                $post = get_post( $row['id_riferimento'] );
                if ( $post ) {
                    $item['title'] = get_the_title( $row['id_riferimento'] );
                    $item['link'] = get_permalink( $row['id_riferimento'] );
                    $item['type'] = $post->post_type === 'page' ? 'Pagina' : 'Articolo';
                    $contents['posts'][] = $item;
                }
            }
        }

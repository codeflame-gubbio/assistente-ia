 ...
 * ✅ FIX v5.6.1: Rate limit multi-livello
 */
class Assistente_IA_Utilita {
    ...
    /** 
     * Rate limit multi-livello per IP+sessione 
     * ✅ FIX v5.6.1: Implementato rate limit su due livelli (sessione + IP globale)
     */
    public static function limita_richieste_utente(string $hash_sessione): void {
        ...
        // Limite per sessione (IP + Hash)
        $k1='assia_rl_session_'.md5($ip.'|'.$hash_sessione);
        $c1=(int)get_transient($k1);
        if($c1>=$max){ 
            wp_send_json_error(['messaggio'=>'Limite richieste raggiunto per questa sessione. Attendi un minuto.']); 
        }
        set_transient($k1,$c1+1,$fin);
        
        // Limite globale per IP (3x più permissivo per prevenire bypass)
        $max_ip = $max * 3;
        $k2='assia_rl_ip_'.md5($ip);
        $c2=(int)get_transient($k2);
        if($c2>=$max_ip){ 
            wp_send_json_error(['messaggio'=>'Limite richieste globale raggiunto. Riprova tra qualche minuto.']); 
        }
        set_transient($k2,$c2+1,$fin);
    }

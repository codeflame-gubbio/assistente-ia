...
class Assistente_IA_Ajax {
    ...
    public function gestisci_chat(){
        ...
        if(empty($messaggio)||empty($hash)||!$this->valida_hash_sessione($hash)) {
            wp_send_json_error(['messaggio'=>'Richiesta non valida']);
        }
        ...
        $res=Assistente_IA_Modello_Vertex::genera_testo($prompt, ['id_chat'=>$id_chat,'hash_sessione'=>$hash]);
    ...
    public function recupera_chat(){
        ...
        $hash=isset($_POST['hash_sessione'])?sanitize_text_field(wp_unslash($_POST['hash_sessione'])):'';
        if(empty($hash)||!$this->valida_hash_sessione($hash)) {
            wp_send_json_error(['messaggio'=>'Hash sessione invalido']);
        }
        $id_chat=$this->ottieni_o_crea_chat($hash);
    ...
    public function storico(){
        ...
        $id_chat = $this->ottieni_o_crea_chat($hash);
        global $wpdb; $pref = $wpdb->prefix;
        $righe = $wpdb->get_results( $wpdb->prepare("SELECT ruolo AS mittente, testo, creato_il FROM {$pref}assistente_ia_messaggi WHERE id_chat=%d ORDER BY id_messaggio DESC LIMIT %d",$id_chat, $limite), ARRAY_A );
        $righe = array_reverse( $righe ?: [] );
        $messaggi = [];
        foreach($righe as $r){
    ...
    public function embeddings_avvia(){
        ...
        if ( ! wp_verify_nonce( $nonce, 'assia_rag_nonce' ) ) { error_log('ASSIA RAG: bad-nonce in embeddings_avvia'); wp_send_json_error(['msg'=>'bad-nonce'], 403); }
    ...
    public function embeddings_step(){
        if ( ! current_user_can('manage_options') ) { wp_send_json_error(['msg'=>'no-cap'], 403); }
        $nonce = isset($_REQUEST['_ajax_nonce']) ? $_REQUEST['_ajax_nonce'] : ( $_REQUEST['nonce'] ?? '' );
        if ( ! wp_verify_nonce( $nonce, 'assia_rag_nonce' ) ) { error_log('ASSIA RAG: bad-nonce in embeddings_step'); wp_send_json_error(['msg'=>'bad-nonce'], 403); }
    ...
    public function embeddings_stato(){
        if ( ! current_user_can('manage_options') ) { wp_send_json_error(['msg'=>'no-cap'], 403); }
        $nonce = isset($_REQUEST['_ajax_nonce']) ? $_REQUEST['_ajax_nonce'] : ( $_REQUEST['nonce'] ?? '' );
        if ( ! wp_verify_nonce( $nonce, 'assia_rag_nonce' ) ) { error_log('ASSIA RAG: bad-nonce in embeddings_stato'); wp_send_json_error(['msg'=>'bad-nonce'], 403); }
    ...
    protected function ottieni_o_crea_chat(string $hash): int {
        ...
        if($id) return $id;
        $wpdb->insert($pref.'assistente_ia_chat',['hash_sessione'=>$hash,'data_creazione'=>current_time('mysql'),'ultimo_aggiornamento'=>current_time('mysql')]);
        return (int)$wpdb->insert_id;
    }

    protected function salva_messaggio(int $id_chat,string $ruolo,string $testo): void {
        global $wpdb; $pref=$wpdb->prefix;
        $wpdb->insert($pref.'assistente_ia_messaggi',['id_chat'=>$id_chat,'ruolo'=>$ruolo,'testo'=>wp_kses_post($testo),'creato_il'=>current_time('mysql')]);

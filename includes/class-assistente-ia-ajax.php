...
class Assistente_IA_Ajax {
    ...
    public function embeddings_avvia(){
        ...
        $job = class_exists('Assistente_IA_RAG_Paged') ? Assistente_IA_RAG_Paged::prepara_job_indicizzazione() : Assistente_IA_RAG::prepara_job_indicizzazione();
    ...
    public function embeddings_stato(){
        check_ajax_referer('assistente_ia_nonce','nonce');
        if ( ! current_user_can('manage_options') ) wp_send_json_error(['messaggio'=>'Permessi insufficienti']);
        $job = class_exists('Assistente_IA_RAG_Paged') ? Assistente_IA_RAG_Paged::stato_job() : Assistente_IA_RAG::stato_job();
        wp_send_json_success( $job );
    }

(function($){
    ...
    // ✅ FIX v5.4.0: Tutte le funzioni dentro il closure
    ...
    function assiaErrore(msg){
        try{
            ...
            if(!box.length){ 
                console.error('Assistente IA:', msg); 
                return; 
            }
            var el = $('<div class="assia-messaggio assia-msg-assistente"></div>').html('<em>' + msg + '</em>');
            box.append(el);
    ...
    // ✅ FIX: Funzione assiaCaricaStorico DENTRO il closure
        ...
        try{
            ...
            .done(function(r){
                if(!r || !r.success || !r.data){ 
                    assiaErrore('Errore: risposta non valida dal server.'); 
                    return; 
                }
            ...
            .fail(function(xhr, status, error){ 
                console.error('Assistente IA - storico fail:', status, error);
                assiaErrore('Errore nel caricamento delle conversazioni precedenti.'); 
            });
        }catch(e){ 
    ...
    // ✅ FIX: Esponi globalmente solo se necessario per chiamate esterne
    ...
    function invia(){
        ...
        $.post(AssistenteIA.ajax_url,{
            ...
            if(!resp||!resp.success){
                assiaErrore('Si è verificato un errore. '+(resp&&resp.data&&resp.data.messaggio?resp.data.messaggio:''));
                return;
            }
        ...
        .fail(function(xhr, status, error){
            rimuovi_preloader();
            console.error('Assistente IA - invio fail:', status, error);
            assiaErrore('Errore di connessione. Riprova.');
        });
    }
